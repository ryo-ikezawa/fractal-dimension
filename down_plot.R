...
calculateFdForPlot <- function(start, end, dirName, loopNumber) {
  ...
  for (i in 1:loopNumber) {
    print(i)
    div = 1
    sample_rate <- 51.2*1000/div
    downsample <- matrix(,1,15360000)
    for (j in 1:15360000) {
#      samplepoint <- div*(j-1)+1+div*15360000*(i-1)
      samplepoint <- (j-1)+1+15360000*(i-1)
...
for (type in 1:4) {
  ...
  if(type==1) {
    ch <- 1
    day <- 0220
    dir <- "../../20190220/ch1/20190220_144135/B4-01_20190220_144135/Wave/WR_B4-01_20190220_144134_0"
    x1 <- calculateFdForPlot(121, 123, dir, 36)
  } else if (type==2) {
    ch <- 2
    day <- 0220
    dir <- "../../20190220/ch2/20190220_144140/B4-01_20190220_144140/Wave/WR_B4-01_20190220_144139_0"
    y1 <- calculateFdForPlot(121, 123, dir, 36)    
  } else if (type==3) {
    ch <- 1
    day <- 0523
    dir <- "../ch1/20190523_120720/B4-01_20190523_120720/Wave/WR_B4-01_20190523_120724_0"
    x2 <- calculateFdForPlot(121, 123, dir, 36)
  } else if (type==4) {
    ch <- 2
    day <- 0523
    dir <- "../ch2/20190523_120626/B4-01_20190523_120626/Wave/WR_B4-01_20190523_120621_0"
    y2 <- calculateFdForPlot(121, 123, dir, 36)
  }
}
...
points(x1, y1, col = "blue", pch = 16, cex=1)
points(x2, y2, col = "orange", pch = 16, cex=1)